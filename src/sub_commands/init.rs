...
pub fn move_git() -> std::io::Result<()> {
    fs::rename(".git", ".git-old")?; // Rename a.txt to b.txt
    Ok(())
}
pub fn move_gnostr() -> std::io::Result<()> {
    fs::rename(".gnostr", ".gnostr-old")?; // Rename a.txt to b.txt
    Ok(())
}
...
pub fn create_initial_commit(repo: &Repository) -> Result<(), Error> {
    ...
    repo.commit(Some("HEAD"), &sig, &sig, "gnostr-cli: initial commit", &tree, &[])?;
...
pub fn create_and_broadcast_init(
    ...
    if git_path.is_file() || git_path.is_dir() {
        //we have already confirmed that the user wants to initialize a new repo
        //we move .git to be non-destructive just in case!
        let _rename = fs::rename(".git", ".git-old");
    }


    // initialise git
    let repo = git2::Repository::init(repo_dir_path.clone()).unwrap();
